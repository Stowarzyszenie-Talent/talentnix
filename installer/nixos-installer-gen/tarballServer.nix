{ system, nixpkgs, pkgs, includeBusybox, inputs ? [], config }:

{
  environment.systemPackages = with pkgs; [ perl perlPackages.JSON nix.perl-bindings python3 config.nix.package nix ];
  system.extraDependencies = [ (pkgs.mkShell { buildInputs = with pkgs; [ perl perlPackages.JSON nix.perl-bindings python3 config.nix.package nix ]; }) ];
  isoImage.contents =
    let
  ...
  systemd.services.mirror-tarballs-symlinks = {
    ...
    path = [ pkgs.bash config.nix.package pkgs.nix ];
  ...
  systemd.services.mirror-tarballs-server = {
    ...
    path = [ pkgs.python3 pkgs.bash config.nix.package pkgs.nix ];
    serviceConfig.User = "tarball-mirror";
